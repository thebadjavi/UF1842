...
function checkGuess() {
  // UF1842: Només cal modificar aquesta funció per aconseguir l'exercici (a més d'alguna variable d'estat). Fixa't que el/la programador/a fa servir moltes variables globals per emmagatzemar els valors de cada component. 
  console.log("que dise "+ guess.value)
  if(guess.value== pokemonName){
    streak++
    console.log("este es "+ pokemonName)
    console.log("este es "+ streak)
    showPokemon();
    ...
    pokemonNameElement.innerHTML = pokemonName; // show Pokemon's name    console.log("fail es "+ pokemonName)
    streak=0
    streakElement.innerHTML = "Ratxa d'encerts: " + streak; // show new streak value
...
function showPokemon() {
  const music = new Audio('whos-that-pokemon_.mp3');
  setTimeout(function(){ 
  ...
  setTimeout(() => {
    getPokemon(); // after showing a pokemon, get the next one
  }, 2000); // wait two seconds before generating new Pokemon and start the same logic again
}
